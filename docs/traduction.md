...
`update.sh` trie les tags avec `sort -V` et prend le plus récent. Un suffixe
`-fr` trie **après** la version nue, donc une install française reste française
et une anglaise reste anglaise **tant que chaque clone suit sa propre branche** —
ce qui est le cas, puisque `git checkout <tag>` ne change jamais de branche.
