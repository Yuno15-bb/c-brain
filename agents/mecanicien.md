...
description: Répare l'infra machine du Claude Brain — hooks, symlinks, capsule, câblage. Jamais le contenu des fiches.
metadata:
  type: reference
...
- Câblage : `~/.claude/settings.json` (les hooks SessionEnd/PostToolUse sont-ils bien enregistrés ?), les **symlinks** (`~/.claude/agents/*`, `~/.claude/projects/-Users-<nom>/memory` → `~/claude-brain`).
...
## Ton processus
0. **Annoncer** (anime la capsule) : `python3 ~/claude-brain/hooks/brain_status.py busy auditing "audit de l'infra"`. Re-pulse selon l'étape ; `… idle` à la fin.
...
6. **Commit** des corrections vérifiées (git, auteur « Claude Brain »). Rapport concis : déjà sain ✓ / corrigé 🔧 / proposé (risqué) ⚠️.
